...
class PlopPlugin : public OFX::ImageEffect {
protected:
...
public:
  /** @brief ctor */
  PlopPlugin(OfxImageEffectHandle handle)
    : ImageEffect(handle)
    , dstClip_(0)
    , srcClip_(0)
    , scale_(0)
...
/// set up and run a processor
void PlopPlugin::setupAndProcess(ImageScalerBase &processor, const OFX::RenderArguments &args)
{
  // get a dst image
...
// override the rod call
bool PlopPlugin::getRegionOfDefinition(const OFX::RegionOfDefinitionArguments &args, OfxRectD &rod)
{
...
// override the roi call
void PlopPlugin::getRegionsOfInterest(const OFX::RegionsOfInterestArguments &args, OFX::RegionOfInterestSetter &rois)
{
...
void PlopPlugin::render(const OFX::RenderArguments &args)
{
...
bool PlopPlugin::isIdentity(const OFX::IsIdentityArguments &args, OFX::Clip * &identityClip, double &identityTime)
{
...
void PlopPlugin::setEnabledness(void)
{
...
void PlopPlugin::changedParam(const OFX::InstanceChangedArgs &/*args*/, const std::string &paramName)
{
...
// we have changed a param
void PlopPlugin::changedClip(const OFX::InstanceChangedArgs &/*args*/, const std::string &clipName)
{
...
mDeclarePluginFactory(PlopPluginFactory, {}, {});

void PlopPluginFactory::describe(OFX::ImageEffectDescriptor& desc)
{
  // basic labels
  desc.setLabels(
    "Plop",
    "Plop",
    "Plop plop plop");
  desc.setPluginGrouping("test/plop");

  desc.setPluginDescription("plop plop plop plop plop");
  ...
  desc.setOverlayInteractDescriptor( new PlopOverlayDescriptor);
}
...
void PlopPluginFactory::describeInContext(OFX::ImageEffectDescriptor& desc, OFX::ContextEnum context)
{
...
ImageEffect *PlopPluginFactory::createInstance(OfxImageEffectHandle handle, ContextEnum /*context*/)
{
  return new PlopPlugin(handle);
}
